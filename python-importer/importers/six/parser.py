...
class SixParser:
    ...
    def parse(self, preventivo_id: str | None = None) -> ParsedEstimate:
        ...
        for key in ordered_keys:
            ...
            # Round quantity to 2 decimals only at final output
            ...
            # If quantity or amount is negligible, zero to avoid -0.00
            ...
            voce = ParsedItem(
                order=len(voci),
                progressive=entry.progressivo,
                code=code,
                description=entry.product.enriched_description or entry.product.description,
                wbs_levels=entry.wbs_levels,
                unit=entry.unita,
                quantity=quantita_float,
                unit_price=prezzo,
                amount=importo,
                notes=note,
        ...
        quantita_values = [voce.quantity for voce in voci if voce.quantity is not None]
        totale_quantita = (
        ...
        return ParsedEstimate(
            ...
            total_amount=totale_importo,
            total_quantity=totale_quantita,
            items=voci,
            stats=dict(stats),
        )
    ...
    def _parse_numeric_value(raw: str | None) -> Decimal | None:
        ...
        try:
            ...
            # Round according to configured measure granularity (0.01 default)
    ...
    def _normalize_measure(value: Decimal | None) -> Decimal | None:
        """Round the product of measures to the configured granularity."""
        if value is None:
            return None
    ...
    def _parse_products(self) -> None:
        for prodotto in self.root.findall(f".//{self.ns}prodotto"):
            ...
            # Check if this is a parent voice (voce="true")
            ...
            for quot in prodotto.findall(f"{self.ns}prdQuotazione"):
                ...
                if existing_priority > priority:
                    if existing_value is not None and existing_value != value:
                        logger.debug(
                        "Price list %s for product %s: ignore new value %s (priority %s < %s)",
                            lista_id,
                            code,
                            value,
                            priority,
                ...
                if (
                    ...
                    logger.debug(
                        "Price list %s for product %s: overwrite value %s with %s (priority %s >= %s)",
                        lista_id,
                        code,
                        existing_value,
                        value,
        ...
        # Enrich descriptions with parent/children
        self._enrich_product_descriptions()

    def _enrich_product_descriptions(self) -> None:
        """
        Enrich product descriptions by concatenating parent and children descriptions.

        Logic:
        - Products with attribute voce="true" are parents (descriptive items)
        - Products whose code starts with the parent code are children
        - Enriched description = parent_description + child_description
        ...
        → Enriched description for 1C.00.700.0010.b:
        ...
        # Find the closest parent (longest prefix)
            parent_description = None
            parent_code_len = 0
            for parent_code, parent_product in parent_products.items():
                # Check if parent is prefix of current code
                ...
                if normalized_code.startswith(parent_code + ".") or \
                   (normalized_code.startswith(parent_code) and len(normalized_code) > len(parent_code)):
                    # Pick parent with longest code (closest in hierarchy)
        ...
        for preventivo in self.root.findall(f".//{self.ns}preventivo"):
            ...
            if rilevazioni_nodes:
                ...
                for rilevazione in rilevazioni_nodes:
                    ...
                    if prezzo is None:
                        continue
                    # Align preview total with main calc: direct quantities + references
