...
const statusFilters = computed(() => {
  ...
  return [
    { key: 'setup', label: 'Setup', tone: 'primary' as const, count: totals.setup },
    { key: 'in_progress', label: 'In progress', tone: 'success' as const, count: totals.in_progress },
    { key: 'closed', label: 'Closed', tone: 'neutral' as const, count: totals.closed },
  ]
})
      ...
      <UCard class="space-y-3 border-border/70 bg-card shadow-sm">
        ...
        <div class="space-y-1">
          ...
          >
            ...
            <UBadge variant="soft" color="neutral" class="font-mono text-[11px]">
              {{ unit.count }}
            </UBadge>
          </UButton>
        </div>
