...
type NavItemWithLink = { label: string; icon: string; to: string }
type NavItemWithClick = { label: string; icon: string; click: () => void }

const profileItems: [(NavItemWithLink | NavItemWithClick)[], NavItemWithClick[]] = [
  [
      ...
      <div class="relative">
        ...
        >
          <button
            ...
            @click="'to' in item ? router.push(item.to) : item.click?.()"
          >
